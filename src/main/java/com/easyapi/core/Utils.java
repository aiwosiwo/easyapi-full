...
import static com.easyapi.core.constant.CoreConstants.JAR_PATH;
import static com.easyapi.core.constant.CoreConstants.USER_DIR;
...
public class Utils {
    ...
    public static boolean isSpringFramework(File javaSrcDir) {
        ...
        Utils.wideSearchFile(javaSrcDir, new FilenameFilter() {
            ...
            public boolean accept(File f, String name) {
                return name.endsWith(".java") && ParseUtils.getCompilationUnit(f).getImports().stream().anyMatch(im -> im.getNameAsString().contains("org.springframework.web"));
            }
        }, result, true);
    ...
    public static boolean isJFinalFramework(File javaSrcDir) {
        ...
        Utils.wideSearchFile(javaSrcDir, new FilenameFilter() {
            @Override
            public boolean accept(File f, String name) {
                return name.endsWith(".java") && ParseUtils.getCompilationUnit(f).getImports().stream().anyMatch(im -> im.getNameAsString().contains("com.jfinal.core"));
            }
        }, result, true);
        return result.size() > 0;
    }
